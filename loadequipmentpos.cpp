...
void loadEquipmentPos::recvBloodPinq16data(const QStringList hexArry)
{
     if(!m_bReadorWrite)
     {
          ...
          QLOG_DEBUG()<<"读清洗样本针的清洗液量:"<<q16cleanbloodauckmm;
...
//读取到的坐标 原点-清洗区样本针-清洗区试剂针-试剂区-丢杯(x)
...
//读取到的坐标 原点-清洗区样本针-清洗区试剂针-试剂区-丢杯(y)
        ...
        for (int t = 0; t < numtray; t++)
        {
             ...
             for (int R = 0; R < 10; R++){
                for (int L = 0; L < 6; L++){
                    ...
                    //QLOG_DEBUG() << "样本针offset试管盘" << keyhole << "坐标=" << tsetTubehole;
                    keyhole++;
                }
            }
        }
        QLOG_DEBUG()<<"读取生成试管区样本针完成";
...
void  loadEquipmentPos::recvTraytubeoffsetHands_y(const QStringList ArryRecvdata)
{
    if(!m_bReadorWrite)
    {
        ...
        QLOG_DEBUG()<<"读取生成试管区样本针完成 nexstep 读AA";
