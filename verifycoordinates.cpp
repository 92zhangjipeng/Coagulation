...
void VerifyCoordinates::printLoadedCoordinates() const
{
    ...
    QLOG_DEBUG() << "样本针坐标:" << m_loadedBloodPin.x() << "," << m_loadedBloodPin.y();
    QLOG_DEBUG() << "清洗区S1(样本针):" << m_loadedCleanZoneBloodPin.x() << "," << m_loadedCleanZoneBloodPin.y();
    QLOG_DEBUG() << "清洗区S1(试剂针):" << m_loadedCleanZoneReagentPin.x() << "," << m_loadedCleanZoneReagentPin.y();
    // ========== 通道坐标输出 ==========
    QLOG_DEBUG() << "=== 抓手通道坐标 ===";
...
bool VerifyCoordinates::verifyCoordinates() const
{
    ...
    validatePoint("样本针坐标", m_loadedBloodPin, m_bloodPin);
    validatePoint("清洗区S1(样本针)", m_loadedCleanZoneBloodPin, m_cleanZoneBloodPin);
    validatePoint("清洗区S1(试剂针)", m_loadedCleanZoneReagentPin, m_cleanZoneReagentPin);
...
QString VerifyCoordinates::getCoordinateDifferences() const
{
    ...
    addDifference(QString::fromUtf8("样本针坐标"), m_loadedBloodPin, m_bloodPin);
    addDifference(QString::fromUtf8("清洗区S1样本针"), m_loadedCleanZoneBloodPin, m_cleanZoneBloodPin);
    addDifference(QString::fromUtf8("清洗区S1试剂针"), m_loadedCleanZoneReagentPin, m_cleanZoneReagentPin);
...
void VerifyCoordinates::startComparingCoordinates(const bool initMachine){
    ...
    if (!verifyCoord->loadCoordinateVerification(equipmentType)) {
        CustomMessageBox::warning(this, "错误",
...
void VerifyCoordinates::displayStyledComparison(const bool initMachine)
{
    ...
    basicPoints["样本针坐标"] = qMakePair(m_bloodPin, m_loadedBloodPin);
    basicPoints["清洗区S1(样本针)"] = qMakePair(m_cleanZoneBloodPin, m_loadedCleanZoneBloodPin);
    basicPoints["清洗区S1(试剂针)"] = qMakePair(m_cleanZoneReagentPin, m_loadedCleanZoneReagentPin);
...
bool VerifyCoordinates::performAxisCalibration()
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        CustomMessageBox::warning(this, "校准失败", QString("无法打开配置文件:\n%1").arg(filePath));
        return false;
    }
...
bool VerifyCoordinates::syncFileToInstrument()
{
    ...
    if (!verifyCoord->loadCoordinateVerification(equipmentName)) {
        QLOG_DEBUG() << "加载配置文件失败";
        CustomMessageBox::warning(this, "同步失败", "无法加载坐标配置文件！");
        return false;
    }
    ...
    QLOG_DEBUG() << QString("同步清洗区S1(样本针)坐标: (%1,%2)").arg(cleanZoneBlood.x()).arg(cleanZoneBlood.y());
    ...
    QLOG_DEBUG() << QString("同步清洗区S1(试剂针)坐标: (%1,%2)").arg(cleanZoneReagent.x()).arg(cleanZoneReagent.y());
