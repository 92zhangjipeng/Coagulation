...
void ThermometerWidget::paintEvent(QPaintEvent *e)
{
    ...
    painter.fillRect(tmpRect, QColor(255, 99, 71));
    ...
    if(h > m_rect.height())
        h = m_rect.height();
    painter.fillRect(m_rect.adjusted(0, m_rect.height()-h-perHeight/2-1 , 0, 0), QColor(255, 99, 71));
...
void ThermometerWidget::startAnimation()
{
    ...
    m_valueAnimation->setKeyValueAt(0.2, curValue+1);
